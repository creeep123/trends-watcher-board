...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ articles: [], total: 0, timestamp: new Date().toISOString() }, { status: 200 });
    const data = await res.json();
    setCache(cacheKey, data);
    setSupabaseCache(cacheKey, data, TTL_MS);
    return NextResponse.json(data);
  } catch {
    return NextResponse.json({ articles: [], total: 0, timestamp: new Date().toISOString() }, { status: 200 });
  }
}
